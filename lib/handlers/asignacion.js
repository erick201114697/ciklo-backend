...
module.exports.register = {
  handler: async (request, h) => {
    ...
    try {
      // /////////////////////////////////////////////////////////////////
      const newUser = new Model.Asignacion(request.payload);
...
module.exports.find = {
  handler: async (request, h) => {
    ...
    try {
      ...
      let user = await Model.Asignacion.findById(request.params.id);
...
module.exports.remove = {
  handler: async (request, h) => {
    ...
    try {
      ...
      let removeUser = await Model.Asignacion.deleteOne({ _id: request.params.id });

      message = utils.message.getMsj(request, httpCode, 15);
      sendClient = CONS.H_ST_OK(httpCode, message, removeUser);
...
module.exports.findAll = {
  handler: async (request, h) => {
    ...
    try {
      ...
      let user = await Model.Asignacion.find();
